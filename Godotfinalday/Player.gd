...
var currSprite = Rect2(0, 1750, 256, 250)
var currSpriteIdle = null
var skin_name = "A" setget set_skin_name

puppet var puppet_skin_name = "" setget puppet_skin_name_set
...
func set_skin_name(value):
	skin_name = value
	if is_network_master():
		rset("puppet_skin_name", value)
	update_shoot_mode(can_shoot)

func puppet_skin_name_set(value):
	puppet_skin_name = value
...
func puppet_hp_set(new_value):
	...
	if get_tree().has_network_peer():
		if not is_network_master():
			hp = puppet_hp
func get_skin_region_for_player(name):
	var skins = {
		"A": 1750,
		"B": 750,
		"C": 0
	}
	return skins.get(name, 0)  # Default
...
func update_shoot_mode(shoot_mode):
	if not shoot_mode:
		sprite.set_region_rect(Rect2(0, get_skin_region_for_player(username_text), 256, 250))
	else:
		#sprite.set_region_rect(Rect2(758, 1750, 256, 250))
		sprite.set_region_rect(Rect2(0, get_skin_region_for_player(username_text), 256, 250))
