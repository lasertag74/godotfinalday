...
func _on_Create_server_pressed():
	if username_text_edit.text != "":
		var skin_picker = $Multiplayer_configure/Skin_picker
		Global.chosen_skin = skin_picker.get_item_text(skin_picker.get_selected())  # 🎨 Save skin choice
...
func _on_Join_server_pressed():
	if username_text_edit.text != "":
		Global.chosen_skin = $Multiplayer_configure/Skin_picker.get_item_text(
			$Multiplayer_configure/Skin_picker.get_selected()
		)
		multiplayer_config_ui.hide()
...
func instance_player(id) -> void:
	...
	player_instance.skin_name = Global.chosen_skin  
	current_spawn_location_instance_number += 1
	if get_tree().is_network_server():
		player_instance.rpc("set_skin_name", Global.chosen_skin)
